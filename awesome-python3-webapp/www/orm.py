...
async def select(sql, args, size=None):
    ...
    async with __pool.get() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            # SQL语句的占位符是?，而MySQL的占位符是 % s，select()
            # 函数在内部自动替换。注意要始终坚持使用带参数的SQL，而不是自己拼接SQL字符串，这样可以防止SQL注入攻击。
            # wwait将调用一个子协程（也就是在一个协程中调用另一个协程）并直接获得子协程的返回结果。
...
# Model只是一个基类，通过metaclass,将具体的子类如User的映射信息读取出来
...
# 定义的是所有ORM映射的基类Model
# Model从dict继承，具备所有dict的功能，同时又实现了特殊方法__getattr__()和__setattr__()，因此又可以像引用普通字段那样写
